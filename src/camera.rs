...
pub fn projection_function(
    ...
    DVec3::new(x_camera, y_camera, -1.0)
}
...
pub fn projection_matrix(
    ...
    let c0 = DVec3::new(coef_x, 0.0, 0.0); // scaled by pixel_x
    let c1 = DVec3::new(0.0, coef_y, 0.0); // scaled by pixel_y
    let c2 = DVec3::new(offs_x, offs_y, -1.0); // added to pixel_x, pixel_y, sets z
    DMat3::new(c0, c1, c2)
}
    ...
    fn test_projection_matrix() {
        ...
        for pixel_x in 0..x_max {
            for pixel_y in 0..y_max {
                ...
                let p_mat = matrix * DVec3::new(pixel_x as f64, pixel_y as f64, 1.0);
                let eps = 1.0e-6;
